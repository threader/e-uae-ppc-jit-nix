...
void SERPER (uae_u16 w)
{
    ...
    if (w & 0x8000) {
        if (!warned) {
            write_log( "SERIAL: program uses 9bit mode\n" );
	    warned++;
        }
        ninebit = 1;
    }
    ...
    write_log ("SERIAL: period=%d, baud=%d, hsyncs=%d\n", w, baud, serial_period_hsyncs);
...
static void checkreceive (int mode)
{
    ...
    if (data_in_serdatr) {
#if 0
        /* not really needed but there might be programs that expect OVRUNs.. */
        overruncnt++;
        if (overruncnt >= 20) {
	    /* wait 20 character periods until overrun is set (1 on real Amiga) */
	    ovrun = 1;
	    overruncnt = 0;
...
#if SERIALDEBUG > 2
    write_log( "SERIAL: wrote 0x%04x (%c)\n", w, dochar (w) );
#endif
...
uae_u16 SERDATR (void)
{
	...
	serdatr |= 0x8000;
#if SERIALDEBUG > 2
    write_log( "SERIAL: read 0x%04.4x (%c)\n", serdatr, dochar (serdatr));
#endif
    ovrun = 0;
    overruncnt = 0;
    data_in_serdatr = 0;
